...
var getHotels = function(searchInput) {
  const options = {
    method: 'GET',
    headers: {
      'X-RapidAPI-Host': 'iraqi-hotel-finder.p.rapi',
      'X-RapidAPI-Key': '53400e456bmsh6c1f0b4a2a22d79p19e3f8jsnb1b9a4a7cc34'
    }
  };

  // Call the hotels.com API with input from the user search
  fetch('https://iraqi-hotel-finder.p.rapidapi.com/app/get/hotel' + searchInput + '&currency=USD&locale=en_US', options)
...
var getHotelImages = function (hotelId) {
    ...
    fetch('https://iraqi-hotel-finder.p.rapidapi.com/app/get/hotel' + hotelId , options)
        .then(response => response.json())
        .then(response => {;
